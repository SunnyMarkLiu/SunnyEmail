package com.markliu.sunnyemail.util.emailutil.service;
...
import com.markliu.sunnyemail.util.emailutil.entities.EmailServerHostAndPort;
import com.markliu.sunnyemail.util.emailutil.entities.EmailServerInfo;
import com.markliu.sunnyemail.util.emailutil.util.FetchingEmailUtil;
	...
	public boolean sendEmail(Session sendMailSession, EmailServerInfo emailServerInfo, EmailInfo email) {
		try {
            ...
            if(!email.getAttachmentFiles().isEmpty()){ // 存在附件  
                for(String fileName : email.getAttachmentFiles().split(";")) { // 遍历所有的附件
                	bodyPart=new MimeBodyPart();  
                    FileDataSource fds=new FileDataSource(fileName); //得到数据源  
                    bodyPart.setDataHandler(new DataHandler(fds)); //得到附件本身并至入BodyPart  
                    bodyPart.setFileName(fds.getName());  //得到文件名同样至入BodyPart
